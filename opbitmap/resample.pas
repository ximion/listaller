...
uses Sysutils, Types, Classes, opbitmap;
...
//procedure Stretch(NewWidth, NewHeight: Cardinal; Filter: TResamplingFilter; Radius: Single; Source, Target: TOPBitmap); overload;
procedure Stretch(NewWidth, NewHeight: Cardinal; Filter: TResamplingFilter; Radius: Single; Source: TOPBitmap); overload;
...
function ClampByte(input:integer):Byte;
begin
if input<0 then Result:=0 else
 if input > $FF then Result:=$FF else
   Result:=input;
end;
...
procedure DoStretch(Filter: TFilterFunction; Radius: Single; Source, Target: TOPBitmap);
...
//----------------------------------------------------------------------------------------------------------------------
{
procedure Stretch(NewWidth, NewHeight: Cardinal; Filter: TResamplingFilter; Radius: Single; Source, Target: TOPBitmap);
...
procedure Stretch(NewWidth, NewHeight: Cardinal; Filter: TResamplingFilter; Radius: Single; Source: TOPBitmap);

var
  Target: TOPBitmap;
  SavedPixelFormat:TPixelFormat;
begin
  if Radius = 0 then Radius := DefaultFilterRadius[Filter];
  Target := TOPBitmap.Create;
  try
  //  SavedPixelFormat:=Source.PixelFormat;
    Target.PixelFormat := pf24Bit;
    Target.Width := NewWidth;
    Target.Height := NewHeight;
    Source.PixelFormat := pf24Bit;
   ...
   // Source.PixelFormat:=SavedPixelFormat;
  finally
    Target.Free;
  end;
end;
