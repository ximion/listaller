...
//  Note that this is a collection of the best Pascal routines for reading and writing
//  image formats and make them work with OPBitmap. Just in case you ask yourself why the design
//  looks inconsistent.
...
Uses
  classes, bitmapimage, jpegpvt, jpegoutputstream, jpegencoderdataunit, jpeghuffmanencoder,
  systemspecific, opbitmap, math;
...
Type
  ...
  TJpegEncoder = Class (TBitmapImageEncoder)
    Private
      ...
      Procedure printSequentialFrame (strm : TJpegOutputStream ; image : TOPBitmap) ;
      Procedure printProgressiveFrame (strm : TJpegOutputStream ; image : TOPBitmap) ;
    ...
    Public
      ...
      Procedure writeImage (strm : TJpegOutputStream ;
                            image : TOPBitmap) ;
      Procedure writeImageFile (filename : String ; image : TOPBitmap) ; {Override ;} {delphi}
      
      Procedure writeImageStream (Stream : TStream ; image : TOPBitmap) ;  //theo
...
//
Procedure TJpegEncoder.writeImage (strm : TJpegOutputStream ; image : TOPBitmap) ;
  Var
    ii : Integer ;
  Begin
  ...
  if comment_string <> '' Then
    printComment (strm, comment_string) else
...
Procedure TJpegEncoder.writeImageFile (filename : String ; image : TOPBitmap) ;
  Var
...
Procedure TJpegEncoder.writeImageStream (Stream : TStream ; image : TOPBitmap) ;
  Var
    outputstream : TJpegOutputFileStream ;
  Begin
  outputstream := TJpegOutputFileStream.Create ;
...
class Procedure TJpegEncoderComponent.rgbConvert (encoder : TJpegEncoder ;
  ...
  for ii := 0 To image.Height - 1 Do
    ...
    for jj := 0 To image.Width - 1 Do
      Begin
      //theo
      pix:= @TBitmapData24(image.Data).RawArray^[pixel];
      valr:=pix^.red;
      valg:=pix^.green;
      valb:=pix^.blue;
      
   {   if ii=0 then
      writeln(valr,' ',valg,' ',valb); }

      yvalue  := RgbToY(valr, valg, valb) ;
      cbvalue := RgbToCb(valr, valg, valb) ;
      crvalue := RgbToCr(valr, valg, valb) ;


    {  yvalue  := RgbToY (pix^.red, pix^.green, pix^.blue) ;
      cbvalue := RgbToCb (pix^.red, pix^.green, pix^.blue) ;
      crvalue := RgbToCr (pix^.red, pix^.green, pix^.blue) ;}

{      yvalue  := RgbToY (JPEGSAMPLE(Min(jj,255)), 140, 129) ;
      cbvalue := RgbToCb (JPEGSAMPLE(Min(jj,255)), 140, 129) ;
      crvalue := RgbToCr (JPEGSAMPLE(Min(jj,255)), 140, 129) ; }
...
{      yvalue  := RgbToY (image.Pixels [pixel].red, image.Pixels [pixel].green, image.Pixels [pixel].blue) ;
      ...
      End ;
    // Extend the last row
    for jja := image.Width To columns - 1 Do
      Begin
      ycomponent.component_buffer [ypointer]   := yvalue ;  Inc (ypointer) ;
      cbcomponent.component_buffer [cbpointer] := cbvalue ; Inc (cbpointer) ;
      crcomponent.component_buffer [crpointer] := crvalue ; Inc (crpointer) ;
  ...
  for iia := image.Height To rows - 1 Do
    Begin
      ...
      for kk := 0 To JPEGSAMPLEWIDTH - 1 Do
        ...
        for ll := 0 To JPEGSAMPLEWIDTH - 1 Do
          ...
          if (row < image.Height) And (column < image.Width) Then
            ...
            pix:= @TBitmapData24(image.Data).RawArray^[pixel];
            yvalue := RgbToY (pix^.red, pix^.green, pix^.blue) ;
         //theo   yvalue := RgbToY (Image.Pixels [pixel].red, Image.Pixels [pixel].green, Image.Pixels [pixel].blue) ;
            data [kk, ll] := yvalue ;
            End
          else
            Begin
