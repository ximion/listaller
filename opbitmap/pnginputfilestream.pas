...
Type
  ...
  TPngInputTStream = Class (TPngInputStream)
    Private
      input_buffer : Array of Char ;
    ...
    Public
      ...
      Function endReached : Boolean ; Override ;
      Procedure readRaw (count :Cardinal ; var buffer : String) ; Override ;
      property Stream:TStream read input_stream write input_stream;
    End ;
...
destructor TPngInputTStream.Destroy;
begin
  inherited Destroy;
end;

function TPngInputTStream.readChunkLength: Cardinal;
  Var
    count : LongInt ;
    Buffer : Cardinal ;
  Begin
...
function TPngInputTStream.readChunkData(chunklength: Cardinal): Pointer;
  var
    Count : LongInt ;
  Begin
  if chunklength > length (input_buffer) Then
...
function TPngInputTStream.readCrc: Cardinal;
  var
    Count : LongInt ;
  Begin
  count := input_stream.read (result, sizeof (result)) ;
...
function TPngInputTStream.endReached: Boolean;
begin
  Result := end_reached ;
end;

procedure TPngInputTStream.readRaw(count: Cardinal; var buffer: String);
var
    ReadCount : LongInt ;
  Begin
  SetLength (buffer, count) ;
