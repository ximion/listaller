...
Type
  ...
  TGifInputTStream = class (TGifInputStream)
    Private
    ...
    Public
      ...
      Procedure FillBuffer ; Override ;
      Function EndReached : Boolean ; Override ;
      property Stream:TStream read input_stream write SetInputStream;
    End ;
...
procedure TGifInputTStream.SetInputStream(const AValue: TStream);
begin
  input_stream:=AValue;
  end_reached := false ;
end;

constructor TGifInputTStream.Create(buffersize: Cardinal);
begin
  Inherited Create ;
  if (buffersize = 0) then
    SetLength (input_buffer, DefaultBufferSize)
...
destructor TGifInputTStream.Destroy;
begin
  inherited Destroy;
end;

function TGifInputTStream.tellg: LongInt;
begin
   Result := LongInt (input_stream.position) ;
end;

procedure TGifInputTStream.seekg(position: LongInt);
begin
  exitBitMode ;
  input_stream.seek (position, soFromBeginning) ;
end;

procedure TGifInputTStream.FillBuffer;
  Var
    count : LongInt ;
  Begin
  count := input_stream.read (input_buffer [0], Length (input_buffer)) ;
...
function TGifInputTStream.EndReached: Boolean;
begin
  Result := end_reached ;
end;
