...
//2007 Theo: This is an adapted version for OpBitmap. The orginal for Delphi is here http://www.delphi-gems.com/
...
uses
 Types, Classes, opbitmap, wincomp, SysUtils,
...
uses
  {$IFNDEF PASBITMAP} Consts, {$ENDIF} Math, MZLib;
...
begin
  ...
  begin
    ...
    begin
      ...
      OPProgress(Self, opsStarting, 0, False, FProgressRect, gesTransfering);
      Read(FileID, 2);
      ...
      begin
        ...
        OPProgress(Self, opsRunning, MulDiv(I, 100, Height), True, FProgressRect, '');
        OffsetRect(FProgressRect, 0, 1);
      end;

      OPProgress(Self, opsEnding, 0, False, FProgressRect, '');
    end;
  end
...
begin
  ...
  begin
    ...
    begin
      ...
      Result := (Swap(Word(Header.Magic)) = SGIMagic) and
...
begin
  ...
  begin
    ...
    begin
      FProgressRect := Rect(0, 0, Width, 1);
      OPProgress(Self, opsStarting, 0, False, FProgressRect, gesPreparing);
      ...
      BlueBuffer := nil;
      AlphaBuffer := nil;
      OPProgress(Self, opsEnding, 100, True, FProgressRect, '');
          ...
          csRGBA:
            begin
              ...
              begin
                ...
                OPProgress(Self, opsRunning, MulDiv(Y, 100, Height), True, FProgressRect, '');
                OffsetRect(FProgressRect, 0, 1);
              end;
            end;
          csRGB:
...
begin
  ...
  begin
    ...
    if Swap(Word(Header.Magic)) = SGIMagic then
      ...
      begin
        ...
        try
          ...
          begin
            ...
            OPProgress(Self, opsRunning, MulDiv(CurrentRow, 100, Height), True, FProgressRect, '');
            OffsetRect(FProgressRect, 0, 1);
          end;
...
begin
  ...
  begin
    ...
    {  if Cardinal(Scanline[0]) > Cardinal(Scanline[1]) then ImageDescriptor := $20
      ...
      ImageDescriptor := $20; //theo
    end;
  
    Stream.Write(Header, SizeOf(Header));

    // store color palette if necessary
    if Header.ColorMapType = 1 then     //theo
    begin
        for I := 0 to 255 do
        begin
         Stream.Write(Pixel32(ColorTable^[i]).Red,1);
         Stream.Write(Pixel32(ColorTable^[i]).Green,1);
         Stream.Write(Pixel32(ColorTable^[i]).Blue,1);
...
//TODO:        GetPaletteEntries(Palette, 0, 256, palPalEntry);
        for I := 0 to 255 do
        begin
    ...
    begin
      ...
      try
        ...
        begin
          ...
          OPProgress(Self, opsRunning, 0, False, FProgressRect, '');
          OffsetRect(FProgressRect, 0, 1);
        end;
      finally
        if Assigned(RLEBuffer) then FreeMem(RLEBuffer);
    ...
    begin
      ...
      try
        ...
        begin
          ...
          C2 := YCbCrData[2];
          I := 0;
          OPProgress(Self, opsStarting, 0, False, FProgressRect, gesLoadingData);
          while I < Rows do
          begin
            OPProgress(Self, opsRunning, MulDiv(I, 100, Rows), False, FProgressRect, '');
          ...
          OPProgress(Self, opsStarting, 0, False, FProgressRect, gesUpsampling);
          ...
          begin
            ...
            try
              ...
              OPProgress(Self, opsRunning, MulDiv(0, 100, Y), False, FProgressRect, '');
              Upsample(768, 512, Width, YCbCrData[0]);
              OPProgress(Self, opsRunning, MulDiv(1, 100, Y), False, FProgressRect, '');
              Upsample(384, 256, Width, YCbCrData[1]);
              OPProgress(Self, opsRunning, MulDiv(2, 100, Y), False, FProgressRect, '');
              ...
              begin
                ...
                OPProgress(Self, opsRunning, MulDiv(3, 100, Y), False, FProgressRect, '');
                Upsample(1536, 1024, Width, YCbCrData[0]);
                OPProgress(Self, opsRunning, MulDiv(4, 100, Y), False, FProgressRect, '');
                Upsample(768, 512, Width, YCbCrData[1]);
                OPProgress(Self, opsRunning, MulDiv(5, 100, Y), False, FProgressRect, '');
                ...
                begin
                  ...
                  OPProgress(Self, opsRunning, MulDiv(6, 100, Y), False, FProgressRect, '');
                  Upsample(3072, 2048, Width, YCbCrData[1]);
                  OPProgress(Self, opsRunning, MulDiv(7, 100, Y), False, FProgressRect, '');
                  Upsample(1536, 1024, Width, YCbCrData[1]);
                  OPProgress(Self, opsRunning, MulDiv(8, 100, Y), False, FProgressRect, '');
          ...
          OPProgress(Self, opsRunning, MulDiv(Y - 1, 100, Y), False, FProgressRect, '');
          Upsample(Width shr 1, Height shr 1, Width, YCbCrData[1]);
          OPProgress(Self, opsRunning, MulDiv(Y, 100, Y), False, FProgressRect, '');
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  if (X mod 8) = 0 then
                  begin
                    Line8^ := Not ReverseBits(Pixel); //Theo
                    Inc(Line8);
                    Pixel := 0;
                  end;
                end;
...
begin
  ...
  begin
    ...
    Frame := Swap(Word(Frame)); //FPC
    Storage_type := Swap(Word(Storage_type)); //FPC  siehe weiter
    Num_chan := Swap(Word(Num_chan));
    Num_matte := Swap(Word(Num_matte));
    Num_aux := Swap(Word(Num_aux));
    Revision := Swap(Word(Revision));
    Job_num  := SwapLong(Job_num);
    Field := Swap(Word(Field));
    Chan_bits := Swap(Word(Chan_bits));
    Matte_type := Swap(Word(Matte_type));
    Matte_bits := Swap(Word(Matte_bits));
    Aux_type := Swap(Word(Aux_type));
    Aux_bits := Swap(Word(Aux_bits));
    Next := SwapLong(Next);
  end;
end;
            ...
            begin
              ...
              begin
                ...
                try
                  ...
                  if PixelFormat=pf1bit then  //theo
                  begin
                  for i := 0 to Data.LineLength * Data.Height do
                    TBitmapData1(Data).RawArray^[i] := Not ReverseBits(TBitmapData1(Data).RawArray^[i]);
                  end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        PixelFormat := TargetPixelFormat;
//theo        PixelFormat := PixelFormatFromBPP(TargetBitsPerSample);
      end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            OPProgress(Self, opsRunning, MulDiv(Row, 100, Height), True, FProgressRect, '');
            OffsetRect(FProgressRect, 0, 1);
          end;
        end;
      end;
