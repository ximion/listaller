...
{$DEFINE USE_MOVE}
{_$DEFINE VER_MINIMAL} // No Resampling, no Canvas Line, Circle... needs less files
...
type
  ...
  TOPProgressStage = (opsStarting, opsRunning, opsEnding);
  
  TOPProgressEvent = procedure (Sender: TObject; Stage: TOPProgressStage;
...
{$ENDIF}
  ...
  private
    ...
    fOnOPProgress:TOPProgressEvent;
...
{$IFNDEF VER_MINIMAL}  procedure SetPalette(const Value: THandle); {$ENDIF}
  ...
  public
    ...
    procedure OPProgress(Sender: TObject; Stage: TOPProgressStage;
       PercentDone: Byte; RedrawNow: Boolean; const R: TRect; const Msg: string); virtual;
...
{$IFNDEF VER_MINIMAL} property Palette: THandle read GetPalette write SetPalette; {$ENDIF}
...
uses Math, {$IFDEF FPC}zstream{$ELSE}ZLib{$ENDIF}
 {$IFNDEF VER_MINIMAL}, resample, ftbresenham, wincomp{$ENDIF};
...
begin
  ...
  if (R = $1F) then R := $FF else if (R <> 0) then R := (R ) shl 3;
  if (G = $1F) then G := $FF else if (G <> 0) then G := (G ) shl 3;
  if (B = $1F) then B := $FF else if (B <> 0) then B := (B ) shl 3;
...
begin
  ...
  if (R = $1F) then R := $FF else if (R <> 0) then R := (R ) shl 3; //(R+1) etc. kann in den extremen besser sein
  if (G = $3F) then G := $FF else if (G <> 0) then G := (G ) shl 2;
  if (B = $1F) then B := $FF else if (B <> 0) then B := (B ) shl 3;
...
var
 ...
 procedure _FreeAndNilFlags;
 var i, j: Cardinal;
 begin
   for j := 0 to $FF do
    for i := 0 to $FF do
    begin
     if Flags[i, j] <> nil then Flags[i, j].Free;
     Flags[i, j] := nil;
    end;
 end;
...
begin
  ...
  for ci := 0 to Size do ColorTable[ci] := 0;
...
procedure TOPBitmap.OPProgress(Sender: TObject; Stage: TOPProgressStage;
  PercentDone: Byte; RedrawNow: Boolean; const R: TRect; const Msg: string);
var DoContinue:Boolean;
begin
 DoContinue:=True;
{$IFDEF IMPORTTGRAPHIC}
 inherited Progress(Sender,Graphics.TProgressStage(Ord(Stage)),PercentDone,RedrawNow,R,Msg,DoContinue);
{$ENDIF}
  if Assigned(FOnOPProgress) then
    FOnOPProgress(Sender, Stage, PercentDone, RedrawNow, R, Msg);
end;
...
function GetBitsPerLine(Width, BitsPerPixel: cardinal;
                        LineEnd: TOpRawImageLineEnd): Cardinal;
var
...
begin
  ...
  begin
    ...
    Result := fMask.Height * (fMask.Width div 8);//LineLength;
  end;
end;
...
procedure TOPBitmapCanvas.Ellipse(X1, Y1, X2, Y2: integer);
begin
  BresenhamEllipse(X1+(X2-X1) div 2, Y1+(Y2-Y1) div 2, (X2-X1) div 2 ,(Y2-Y1) div 2, self);
end;
{$ENDIF}
...
begin
  ...
  begin
    ...
    if (Canvas.fBitmap.PixelFormat = pf32bit) and (fBitmap.PixelFormat = pf32bit) then  //new
      for y := 0 to Hei - 1 do
        Move(TBitmapData32(Canvas.fBitmap.Data).RawArray^[(S.Top + y) * Canvas.fBitmap.Width + S.Left],
          TBitmapData32(fBitmap.Data).RawArray^[(D.Top + y) * fBitmap.Width + D.Left], Wid * 4)
    else
      for y := 0 to Hei - 1 do
        for x := 0 to Wid - 1 do
...
begin
  ...
  Assert(Canvas.fBitmap.PixelFormat = pf32bit, '32 BPP only');
  Assert(fBitmap.PixelFormat = pf32bit, '32 BPP only');
  S := Source;
  D := Dest;
  ...
  Hei := Min(D.Bottom - D.Top, S.Bottom - S.Top);
    ...
    swid:= Canvas.fBitmap.Width;
    dwid:=fBitmap.Data.Width;
    sparr:=TBitmapData32(Canvas.fBitmap.Data).fPixels;
    dparr:=TBitmapData32(fBitmap.Data).fPixels;
    for y := 0 to Hei - 1 do
      for x := 0 to Wid - 1 do
      begin
