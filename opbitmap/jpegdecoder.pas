...
Uses classes, sysutils, bitmapimage, jpeginputstream,
     jpegpvt, jpeghuffmandecoder, jpegdecoderdataunit, opbitmap ;
...
Type
  ...
  TJpegDecoder = Class (TBitmapImageDecoder)
    ...
    public
      ...
      Procedure readImageFile (filename : String ; image : TOPBitmap) ;
      Procedure readImageStream (Stream:TStream; image :  TOPBitmap) ;  //theo
      Procedure readImage (inputstream : TJpegInputStream ; image : TOPBitmap) ; Virtual ;
...
//
Procedure TJpegDecoder.readImage (inputstream : TJpegInputStream ; image : TOPBitmap) ;
  Const
    CR = 13 ;
    LF = 10 ;
  Var
 ...
 Procedure readApplication (marker : Byte ; inputstream : TJpegInputStream) ;    //Theo with Pchar
      Const
      ...
      id := StrAlloc(length);
      inputstream.read (id^, length - Sizeof (WORD)) ;
      fComment:=id;
      StrDispose(id);
...
//      current_image.setSize (frame_width, frame_height) ;
      current_image.Width:=Frame_width;
      current_image.Height:=Frame_height;      
...
Procedure TJpegDecoder.readImageStream (Stream:TStream; image :  TOPBitmap) ;
  Var
    inputstream : TJpegInputTStream ;
//    JpegInputMapStream inputstream ;
  Begin
  inputstream := TJpegInputTStream.Create (BUFFERSIZE) ;
  try
    try
      Stream.position:=0;
...
Class Procedure TJpegDecoderComponent.convertGrayscale (
  ...
  For ii := 1 To image.Height Do
    ...
    for jj := 1 To image.Width Do
      ...
      Pix^.red   := cc.upsample_data [offset] ;
      Pix^.green := cc.upsample_data [offset] ;
      Pix^.blue  := cc.upsample_data [offset] ;
//      Pix^.alpha := $FF ;                           //theo
...
Class Procedure TJpegDecoderComponent.convertRgb (
  ...
  for ii := 1 To image.Height Do
    ...
    for jj := 1 To image.Width Do
      ...
      pix:= @TBitmapData24(image.Data).RawArray^[pixel];
      Pix^.red := YCbCrToR (yvalue, cbvalue, crvalue) ;
      Pix^.green := YCbCrToG (yvalue, cbvalue, crvalue) ;
      Pix^.blue := YCbCrToB (yvalue, cbvalue, crvalue) ;
//      Pix^.alpha := $FF ;
{      image.Pixels [pixel].red := YCbCrToR (yvalue, cbvalue, crvalue) ;
      ...
      image.Pixels [pixel].alpha := $FF ; }  //theo
      Inc (offset) ;
      Inc (pixel) ;
      End ;
...
Class Procedure TJpegDecoderComponent.convertCymk (decoder : TJpegDecoder ;
  ...
  for ii := 1 To image.Height Do
    ...
    for jj := 1 To image.Width Do
      ...
      pix:= @TBitmapData24(image.Data).RawArray^[pixel];
      Pix^.red   := Clamp (cvalue - (JPEGMAXSAMPLEVALUE - kvalue)) ;
      Pix^.green := Clamp (mvalue - (JPEGMAXSAMPLEVALUE - kvalue)) ;
      Pix^.blue  := Clamp (yvalue - (JPEGMAXSAMPLEVALUE - kvalue)) ;
//      Pix^.alpha := $FF ;
