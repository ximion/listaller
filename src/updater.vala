	...
	private bool perform_application_update (AppItem app_old, AppItem app_new, string arch) {
		...
		if (inst == null) {
			emit_error (ErrorEnum.UPDATE_FAILED,
				_("Update of application '%s' failed. Maybe the update information is out of date. Please refresh it and try again.").printf (app_old.metainfo.name));
			return false;
		}
		...
		if (!ret) {
			emit_error (ErrorEnum.UPDATE_FAILED,
				_("Update of application '%s' failed. Unable to remove the old application.").printf (app_old.metainfo.name));
			return false;
		}
		...
		if (!ret) {
			...
			emit_error (ErrorEnum.UPDATE_FAILED,
				_("Update of application '%s' failed. Something bad happened. Please install the application again!").printf (app_old.metainfo.name));
			return false;
		}
